...
class CDiskFolderListComponent extends DiskComponent implements \Bitrix\Main\Engine\Contract\Controllerable
{
	...
	private function modifyByFilter(array $parameters)
	{
        ...
        if ( !empty( $request['tag'] ) ) {
            $filterData['TAGS'] = $request['tag'];
            $filterData['PRESET_ID'] = 'tmp_filter';
            $filterData['FILTER_ID'] = 'tmp_filter';
            $filterData['FILTER_APPLIED'] = 1;
            $filterData['FIND'] = $request['tag'];

            //$arOptions['filters']['tmp_filter']['fields']['TAGS'] = $request['tag'];
        }
        ...
        if (Loader::includeModule("iblock")) {
            if ( !empty($filterData['TAGS']) ) {
                $request = Bitrix\Main\Context::getCurrent()->getRequest();
                if ( !empty( $request['tag'] ) &&  $request['tag'] != $filterData['TAGS'] ) {
                    $filterData['TAGS'] = $request['tag'];
                }
                ...
                $arFilter = array(
                    ...
                    "ACTIVE" =>"Y",
                    "NAME" => $filterData['TAGS']
                );
                $arSelectFields = array( "ID", "NAME", "PROPERTY_FILE_ID" );

                $rsTags = \CIBlockElement::GetList(array(), $arFilter,  false, array(), $arSelectFields );
                if ( $rsTags ) {
                    $arModifyTagField = array();
                    while($arTagFields = $rsTags->fetch()) {
                        $arModifyTagField[] = $arTagFields['PROPERTY_FILE_ID_VALUE'];
                    }

                    if ( !empty( $arModifyTagField ) )
                        $parameters['filter']['ID'] = $arModifyTagField;
                }

                $filterData['FIND'] = '';
                unset( $filter['*SEARCH_INDEX'] );
            }
        }
	...
	protected function getGridHeaders()
	{
		...
		$headers = array(
            ...
            array(
                'id' => 'TAGS',
                'name' => 'TAGS',
                'sort' => isset($possibleColumnForSorting['TAGS']) ? 'TAGS' : false,
                'default' => isset($defaultColumns['TAGS']),
            ),
			array(
				'id' => 'NAME',
