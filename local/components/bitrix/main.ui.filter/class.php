...
class CMainUiFilter extends CBitrixComponent
{
	...
	protected function applyOptions()
	{
        ...
        if ( !empty( $request['tag'] ) ) {
            $arFilter['TAGS'] = $request['tag'];
            $arFilter['PRESET_ID'] = 'tmp_filter';
            $arFilter['FILTER_ID'] = 'tmp_filter';
            $arFilter['FILTER_APPLIED'] = 1;
            $arFilter['FILTER_APPLIED'] = $request['tmp_filter'];
		...
		if (!empty($optionsPresets) && is_array($optionsPresets))
		{
            ...
            $request = Bitrix\Main\Context::getCurrent()->getRequest();
            if ( !empty( $request['tag'] ) ) {
                //$this->arResult["CURRENT_PRESET"]["FIND"] = $request['tag'];

                foreach ( $this->arResult['PRESETS'] as $arKeyPresets => $arItemPresets ) {
                    ...
                    if ( $arItemPresets['ID'] == 'tmp_filter' ) {
                        foreach ( $arItemPresets['FIELDS'] as $arKeyPresetFields => $arPresetFields ) {
                            if ( $arPresetFields['ID'] == 'field_TAGS' ) {
                                ...
                                $this->arResult['PRESETS'][ $arKeyPresets ]['FIELDS'][ $arKeyPresetFields ]['VALUE'] = $request['tag'];
                            }
                        }
                    }
                }

                foreach ( $this->arResult['CURRENT_PRESET']['FIELDS'] as $arKeyCurrentPreset => $arCurrentPresetField ) {
                    if ( $arCurrentPresetField['ID'] == 'field_TAGS' ) {
                        $arCurrentPresetField['VALUE'] = $request['tag'];
                        $this->arResult['CURRENT_PRESET']['FIELDS'][ $arKeyCurrentPreset ]['VALUE'] = $request['tag'];
                    }
                }
            }
